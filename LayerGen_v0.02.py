...
class ProviderClient:
    ...
    def _generate_local_gguf(self, model_name, prompt):
        ...
        if not model_path:
            raise RuntimeError("Choose a local GGUF model file.")
        ...
        try:
            ...
            raise RuntimeError("The local model run timed out.")

        output = (completed.stdout or "").strip()
        error = (completed.stderr or "").strip()
        if completed.returncode != 0:
            detail = error or output or "No output returned."
            raise RuntimeError("Local model run failed: " + detail[:1200])
        if output:
            return output
        if error:
            return error
        raise RuntimeError("The local model did not return output.")
...
class LayerPanel:
    ...
    def _build_ui(self):
        ...
        self.prompt_text.bind("<KeyRelease>", lambda event: self.app.schedule_flowchart_refresh())
...
class LayerGenApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.main_notebook.grid(row=2, column=0, sticky="nsew", padx=10)
        self.main_notebook.bind("<<NotebookTabChanged>>", self._main_tab_changed)

        layers_frame = ttk.Frame(self.main_notebook, padding=4)
        ...
        log_frame = ttk.Frame(self.main_notebook, padding=4)
    ...
    def _draw_flow_node(self, canvas, box, title, subtitle, fill, outline):
        x1, y1, x2, y2 = box
        canvas.create_rectangle(x1, y1, x2, y2, fill=fill, outline=outline, width=1.5)
        canvas.create_text(
            x1 + 12,
            y1 + 16,
